...
2.  다른 사람들의 풀이에서는 시간을 효율적으로 사용하기 위해,
    이차원배열을 1차원으로 풀어 계산하고 복사 과정도 원본 배열에서 값을 빼고
    새 배열의 계산된 값을 원본 배열에 더해주는 방식으로 deepcopy에 드는 시간을
    줄이는 방식을 채택. 만약 시간이 부족할 경우 고려해 볼만한 방법.
"""
